#!/usr/bin/env python3
"""
mythos_3b round 2.3 — joint backbone + head fine-tune with PonderNet-style
geometric KL regulariser.

The two-stage recipe (round 2.1/2.2 ACT-bypass training + head-only
anti-collapse fine-tune) recovered a non-trivially-halting head but only
by pushing it to the OTHER trivial extreme (p ~ 0.01-0.05, never halt).
A head-only PonderNet retrain sweep (LAMBDA_KL in {1, 2, 5}) confirmed
that with a frozen bypass-trained backbone, no stable intermediate
equilibrium exists at the geometric target: either CE wins (p settles
0.5-0.7) or KL dominates and the head crashes through target to p ~ 0.

Round 2.3 trains backbone AND head jointly with ACT enabled and the
PonderNet KL regulariser. The recurrent forward is the standard
ACT-weighted form (NOT the bypass). The KL term shapes the realised
halt distribution toward Geom(LAMBDA_P).

Loss:
  L = CE(logits, targets) + LAMBDA_KL * sum_t KL(Bern(p_t) || Bern(LAMBDA_P))

KL is summed over recurrent iterations and averaged over (B, T_seq).
LAMBDA_P=0.2 targets a mean halt step of 5. With backbone trainable, the
joint optimisation should make p ~ LAMBDA_P stable (the backbone learns
to produce useful output at the target halt step).

Bootstrap:
  - Honours `BOOTSTRAP_CKPT` env var as an explicit override (used by
    round 2.4 to bootstrap from the round-2 collapsed ckpt and test
    whether the bypass interlude was load-bearing).
  - Otherwise loads the round-2.2 final consolidated ckpt (auto-discovers
    the latest `checkpoints_3b_varT_act_v3/step_*_full.pt`), falling back
    to round-2.1 final if 2.2 has not been consolidated.
  - Re-initialises ACTHalting.halt so the saturated round-2-inherited
    head does not dominate the early KL gradient direction.

Hyperparameters (env-overridable):
  TARGET_TOKENS  default 50000000
  MICRO_BATCH    default 1
  GRAD_ACCUM     default 4
  CKPT_EVERY     default 200
  LR             default 5e-5     (lower than round 2.2 — joint optim
                                    is more sensitive)
  WARMUP_STEPS   default 200
  LAMBDA_KL      default 1.0
  LAMBDA_P       default 0.2
  REINIT_HEAD    default 1
"""
...
def main():
    ...
    # Round 2.3 uses lower peak LR than round 2.2 because joint optimisation
    # of backbone + ACT head (with KL regulariser) is more sensitive than
    # pure bypass training; 5e-5 is half of round 2.2's 1e-4.
    warmup_steps = int(os.environ.get("WARMUP_STEPS", "200"))
    lr = float(os.environ.get("LR", "5e-5"))
    wd = 0.1
    log_every = 5
    ckpt_every = int(os.environ.get("CKPT_EVERY", "200"))
    ckpt_dir = os.environ.get("CKPT_DIR", "checkpoints_3b_varT_pondernet_joint")
    dataset_subset = os.environ.get("DATASET_SUBSET", "sample-10BT")
    # PonderNet KL: per-iteration Bernoulli KL between p_t and lambda_p.
    # lambda_kl is the weight in the joint loss; lambda_p is the geometric
    # halt prior (mean halt step ~= 1/lambda_p). Empirically, lambda_kl=1
    # is too weak (CE wins, head saturates) and lambda_kl=5 too strong
    # (head crashes through target). Default lambda_kl=2 sits in the
    # bistable regime when backbone is frozen; with backbone trainable
    # the equilibrium should be tighter.
    lambda_kl = float(os.environ.get("LAMBDA_KL", "1.0"))
    lambda_p = float(os.environ.get("LAMBDA_P", "0.2"))
    reinit_head = os.environ.get("REINIT_HEAD", "1") == "1"
    ...
    if master:
        ...
        logger.info(
            f"PonderNet KL: lambda_kl={lambda_kl} | lambda_p={lambda_p} "
            f"(target mean halt step {1.0 / lambda_p:.1f}) | reinit_head={reinit_head}"
        )
    ...
    # Round 2.3 uses the standard ACT-weighted recurrent forward (no bypass).
    # Both backbone and head receive gradient through the ACT path; the
    # PonderNet KL regulariser shapes the head's per-iteration halting
    # probabilities toward Geom(LAMBDA_P).
    if master:
        logger.info(
            "ACT enabled (no bypass); PonderNet KL regulariser active"
        )
    ...
    #   1. Existing sharded ckpts in `ckpt_dir` (continuing a round-2.3 run)
    #   2. Latest consolidated full-state-dict in checkpoints_3b_varT_act_v3/
    #      (round 2.2 final — primary bootstrap for round 2.3)
    ...
    if decision.mode == "resume_shards":
        ...
        if decision.mode == "bootstrap_full":
            ...
            if v3_candidates:
                bootstrap_path = v3_candidates[-1]
                bootstrap_label = "round-2.2 final"
            elif v2_candidates:
                bootstrap_path = v2_candidates[-1]
                bootstrap_label = "round-2.1 final (fallback; v2.2 not consolidated)"
        ...
        if bootstrap_path is not None:
            ...
            start_step = 0
            # Re-initialise the ACT head so it starts away from the
            # round-2-inherited saturated state. Without re-init, the
            # head emits p_t = 1.0 from step 0 and the KL gradient is
            # huge, dominating the early backbone updates and likely
            # destabilising training. With re-init the head starts near
            # p ~ 0.5 and the KL/CE forces compete from a neutral base.
            if reinit_head:
                if ddp:
                    with FSDP.summon_full_params(model, writeback=True):
                        model.recurrent.act.halt.reset_parameters()
                else:
                    model.recurrent.act.halt.reset_parameters()
                if master:
                    logger.info("ACT head re-initialised")
            if master:
                logger.success(
                    f"Loaded weights from {bootstrap_label}; "
                    "starting round-2.3 step counter at 0"
                )
        else:
            if master:
                logger.warning(
    ...
    # FSDP-friendly hook: register on the underlying ACTHalting module so
    # we capture per-iteration p tensors without breaking FSDP's parameter
    # gathering. The hook fires once per recurrent iteration during forward.
    # We keep the live tensors (no detach) so the KL loss's gradient flows
    # back to the head's nn.Linear.
    ...
    while step < total_steps:
        ...
        for micro_step in range(grad_accum):
            ...
            with sync, amp_ctx:
                ...
                # PonderNet KL: per-iteration Bernoulli KL between the
                # head's emitted p_t and Bern(lambda_p). Summed over
                # iterations, mean over (B, T_seq). Drives p_t toward
                # lambda_p (target geometric prior, mean halt 1/lambda_p).
                if captured_p:
                    p_stack = torch.stack(captured_p, dim=0).float().clamp(
                        1e-6, 1.0 - 1e-6
                    )
                    kl_per_iter = (
                        p_stack * (torch.log(p_stack) - log_lambda_p)
                        + (1.0 - p_stack) * (
                            torch.log(1.0 - p_stack) - log_one_minus_lambda_p
                        )
                    )
                    kl = kl_per_iter.sum(dim=0).mean()
                else:
                    kl = torch.zeros((), device=ce.device, dtype=ce.dtype)
                loss = (ce + lambda_kl * kl) / grad_accum

            loss.backward()
            loss_accum += float(loss.item())
            ce_accum += float(ce.item()) / grad_accum
            kl_accum += float(kl.item()) / grad_accum
        ...
        if master and step % log_every == 0:
            ...
            mean_p1 = float(captured_p[0].detach().mean().item()) if captured_p else 0.0
            logger.info(
                f"step {step:6d}/{total_steps} | loss {loss_accum:.4f} "
                f"| ce {ce_accum:.4f} | kl {kl_accum:.4f} | p_1 {mean_p1:.3f} "
