#!/usr/bin/env bash
# auto_eval_round27.sh
#
# Watcher that runs on kebab-spark.lan. Waits for round 2.7 training to print
...
#                              (key for paper: did PonderNet KL alone
#                              rescue the collapsed head, or did it
#                              re-saturate?)
...
#                            round27_*.png so prior figures survive)
#   7. build_paper_section7.py   fills docs/paper/round27_results.md
#                                with the empirical results
#
# Round 2.4 trains PonderNet joint from the round-2 collapsed (LAMBDA_P=0.1) ckpt
# (checkpoints_3b_varT_fast/step_0012207_full.pt) without the round-2.1/2.2
# bypass interlude. This isolates whether the bypass step is necessary or
# whether the freshly-init'd halting head + KL prior alone can rescue a
# collapsed backbone.
#
# Output:
#   /home/alexm/OpenMythos/checkpoints_3b_varT_pondernet_round27_lp01/step_NNNNNNN_full.pt
#   /home/alexm/OpenMythos/docs/depth_extrap_round27.json
#   /home/alexm/OpenMythos/docs/act_halt_diagnostic_round27.json
#   /home/alexm/OpenMythos/docs/act_halt_histogram_round27.json
#   /home/alexm/OpenMythos/docs/gen_samples_round27_multidepth.txt
#   /home/alexm/OpenMythos/docs/paper/figures/round27_training_curve.png
#   /home/alexm/OpenMythos/docs/paper/figures/round27_loss_by_T.png
#   /home/alexm/OpenMythos/docs/paper/round27_results.md

set -uo pipefail

LOG=/home/alexm/OpenMythos/training/auto_eval_round27.log
TRAIN_LOG=/tmp/train_r0.log
CKPT_DIR=/home/alexm/OpenMythos/checkpoints_3b_varT_pondernet_round27_lp01
...
# ----------------------------------------------------------------------
# 1. Wait for round 2.7 completion. Tail forward only so we don't trigger
#    on the round-2.3 "Training complete." line that may still be in the
#    file when round 2.7 starts (launch_3b.sh truncates /tmp/train_r0.log
#    on each fresh launch, but be defensive).
# ----------------------------------------------------------------------
log "auto_eval_round27 watcher started; tailing $TRAIN_LOG for 'Training complete.'"
tail -F -n 0 "$TRAIN_LOG" 2>/dev/null | grep -m 1 'Training complete\.'
log "round 2.7 completion detected"
...
log "round 2.7 final ckpt step=${STEP}"
...
for HOST in kebab-spark-200g kebab-gx10-200g kebab-gx10-2-200g kebab-gx10-3-200g; do
  ...
  ssh -q -o StrictHostKeyChecking=accept-new "alexm@$HOST" \
    "rm -f /tmp/consolidate_round27_r${RANK}.log; cd ~/OpenMythos && nohup env $NCCL_BASE NCCL_IB_GID_INDEX=${GID} \
     torchrun --nnodes=4 --nproc_per_node=1 --node_rank=${RANK} \
       ...
       >/tmp/consolidate_round27_r${RANK}.log 2>&1 </dev/null & disown" &
  RANK=$((RANK+1))
done
wait
...
while [ ! -s "$DST_PATH" ] || [ -f "${DST_PATH}.tmp" ]; do
  if [ "$(date +%s)" -gt "$DEADLINE" ]; then
    ...
    log "rank 0 log tail:"
    tail -n 40 /tmp/consolidate_round27_r0.log 2>&1 | tee -a "$LOG"
    exit 1
  fi
  sleep 5
done
...
# training_curves.py uses hardcoded paths; run, then move to round27 names
# inside docs/paper/figures/. Prior round figures are preserved.
log "training_curves.py (then rename PNGs into paper figures dir)"
python3 training/training_curves.py 2>&1 | tee -a "$LOG"
mv -v "$DOCS_DIR/training_curve.png" "$PAPER_FIG_DIR/round27_training_curve.png" 2>&1 | tee -a "$LOG"
mv -v "$DOCS_DIR/loss_by_T.png" "$PAPER_FIG_DIR/round27_loss_by_T.png" 2>&1 | tee -a "$LOG"

# ----------------------------------------------------------------------
# 5. Generate the paper §7 fragment
# ----------------------------------------------------------------------
PAPER_FRAG=$DOCS_DIR/paper/round27_results.md
...
[$(date '+%F %T')] auto_eval_round27 pipeline complete.
  ...
  Training curves:         ${PAPER_FIG_DIR}/round27_*.png
  Paper section fragment:  ${PAPER_FRAG}

Next step: open ${PAPER_FRAG} and integrate the ablation result into
docs/paper/main.md (round 2.7 = PonderNet from r2 collapsed; tests
whether the bypass interlude was necessary).
EOF
