#!/usr/bin/env bash
# auto_eval_round214.sh
#
# Watcher that runs on kebab-spark.lan. Waits for round 2.14 training
# (T_FIXED=16 from r2 collapsed, compute scaling extension) to print
# "Training complete." in /tmp/train_r0.log, then drives the post-training
# pipeline:
#
#   1. 4-rank torchrun consolidate_ckpt.py
#   2. depth_extrap.py
#   3. act_halt_diagnostic.py (head saturated -- bypass training)
#   4. act_halt_histogram.py
#   5. gen_samples_multidepth.py
#   6. build_paper_section7.py -> docs/paper/round214_results.md

set -uo pipefail

LOG=/home/alexm/OpenMythos/training/auto_eval_round214.log
TRAIN_LOG=/tmp/train_r0.log
CKPT_DIR=/home/alexm/OpenMythos/checkpoints_3b_varT_act_v3_round214_T16
...
log "auto_eval_round214 watcher started; checking $TRAIN_LOG for 'Training complete.'"
...
log "round 2.14 completion detected"
...
log "round 2.14 final ckpt step=${STEP}"
...
for HOST in kebab-spark-200g kebab-gx10-200g kebab-gx10-2-200g kebab-gx10-3-200g; do
    ...
    ssh -q -o StrictHostKeyChecking=accept-new "alexm@$HOST" \
        "rm -f /tmp/consolidate_round214_r${RANK}.log; cd ~/OpenMythos && nohup env $NCCL_BASE NCCL_IB_GID_INDEX=${GID} \
         torchrun --nnodes=4 --nproc_per_node=1 --node_rank=${RANK} \
           ...
           >/tmp/consolidate_round214_r${RANK}.log 2>&1 </dev/null & disown" &
    RANK=$((RANK+1))
done
wait
...
while [ ! -s "$DST_PATH" ] || [ -f "${DST_PATH}.tmp" ]; do
    if [ "$(date +%s)" -gt "$DEADLINE" ]; then
        ...
        tail -n 40 /tmp/consolidate_round214_r0.log 2>&1 | tee -a "$LOG"
        exit 1
    fi
    sleep 5
done
...
[$(date '+%F %T')] auto_eval_round214 pipeline complete.
...
Extends the compute scaling curve T=1/2/4/8 with T=16 to test whether
deeper recurrence continues to help or shows diminishing returns.
EOF
