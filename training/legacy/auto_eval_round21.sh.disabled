#!/usr/bin/env bash
# auto_eval_round21.sh
#
# Watcher that runs on kebab-spark.lan. Waits for round 2.1 training to print
...
#                            round21_*.png so round 2 figures survive)
#   7. build_paper_section7.py   fills docs/paper/round21_results.md
#                                with the empirical §7 numbers
#
# Output:
#   /home/alexm/OpenMythos/checkpoints_3b_varT_act_v2/step_NNNNNNN_full.pt
#   /home/alexm/OpenMythos/docs/depth_extrap_round21.json
#   /home/alexm/OpenMythos/docs/act_halt_diagnostic_round21.json
#   /home/alexm/OpenMythos/docs/act_halt_histogram_round21.json
#   /home/alexm/OpenMythos/docs/gen_samples_round21_multidepth.txt
#   /home/alexm/OpenMythos/docs/paper/figures/round21_training_curve.png
#   /home/alexm/OpenMythos/docs/paper/figures/round21_loss_by_T.png
#   /home/alexm/OpenMythos/docs/paper/round21_results.md
#
# Launch on spark with:
#   ssh kebab-spark.lan "nohup bash ~/OpenMythos/training/auto_eval_round21.sh \\
#                       >/tmp/auto_eval_round21.log 2>&1 </dev/null & disown; echo \$!"

set -uo pipefail

LOG=/home/alexm/OpenMythos/training/auto_eval_round21.log
TRAIN_LOG=/tmp/train_r0.log
CKPT_DIR=/home/alexm/OpenMythos/checkpoints_3b_varT_act_v2
...
# ----------------------------------------------------------------------
log "auto_eval_round21 watcher started; tailing $TRAIN_LOG for 'Training complete.'"
tail -F -n 0 "$TRAIN_LOG" 2>/dev/null | grep -m 1 'Training complete\.'
log "round 2.1 completion detected"
...
log "round 2.1 final ckpt step=${STEP}"
...
for HOST in kebab-spark-200g kebab-gx10-200g kebab-gx10-2-200g kebab-gx10-3-200g; do
  ...
  ssh -q -o StrictHostKeyChecking=accept-new "alexm@$HOST" \
    "rm -f /tmp/consolidate_round21_r${RANK}.log; cd ~/OpenMythos && nohup env $NCCL_BASE NCCL_IB_GID_INDEX=${GID} \
     torchrun --nnodes=4 --nproc_per_node=1 --node_rank=${RANK} \
       ...
       >/tmp/consolidate_round21_r${RANK}.log 2>&1 </dev/null & disown" &
  RANK=$((RANK+1))
done
wait
...
while [ ! -s "$DST_PATH" ] || [ -f "${DST_PATH}.tmp" ]; do
  if [ "$(date +%s)" -gt "$DEADLINE" ]; then
    ...
    log "rank 0 log tail:"
    tail -n 40 /tmp/consolidate_round21_r0.log 2>&1 | tee -a "$LOG"
    exit 1
  fi
  sleep 5
done
...
# training_curves.py uses hardcoded paths; run, then move to round21 names
...
mv -v "$DOCS_DIR/training_curve.png" "$PAPER_FIG_DIR/round21_training_curve.png" 2>&1 | tee -a "$LOG"
mv -v "$DOCS_DIR/loss_by_T.png" "$PAPER_FIG_DIR/round21_loss_by_T.png" 2>&1 | tee -a "$LOG"

# ----------------------------------------------------------------------
# 5. Generate the paper §7 fragment
# ----------------------------------------------------------------------
PAPER_FRAG=$DOCS_DIR/paper/round21_results.md
...
[$(date '+%F %T')] auto_eval_round21 pipeline complete.
  ...
  Training curves:         ${PAPER_FIG_DIR}/round21_*.png
