#!/usr/bin/env python3
"""
mythos_3b round 2.1 — fix the ACT-collapse failure mode from round 2.

Round-2 diagnostic (docs/act_halt_diagnostic_round2.json) showed ACTHalting
saturated to p=1.0 for every (batch, position, iteration). Variable-T training
under the original ACT-weighted h_out objective gave the model a free shortcut:
emit p=1 at iter 1, h_out = h_iter_1, gradient-through-the-rest goes to zero,
model never learns depth dependency.

Round 2.1 removes the shortcut by bypassing ACT entirely during training. The
recurrent loop runs all T iterations and returns h_T directly. Loss flows
through every iteration, so the model is forced to learn that more loops
produce a better h_T. ACT can be re-enabled at inference (with a head reset
or fine-tune) once depth-using behavior is established.

Changes vs round 2:
  - Monkey-patches RecurrentBlock.forward to skip ACT and return h_T
  - target_tokens = 50M (regime test, not full pretraining)
  - peak LR = 1e-4 (continuing from a cosine-decayed ckpt, no re-spike)
  - warmup_steps = 200 (short; model is near optimum)
  - Resumes from checkpoints_3b_varT_fast/step_0012207_full.pt (round-2 final)
  - Separate ckpt dir (checkpoints_3b_varT_act_v2)

Inherits from round 2:
  - Variable T sampled uniformly from [T_MIN, T_MAX] per optimizer step
  - FSDP SHARD_GRAD_OP + bf16 mixed precision
  - micro_batch=1, grad_accum=4
  - Sharded checkpoints
"""
...
def main():
    ...
    target_tokens = 50_000_000  # round 2.1 regime test; extend if model uses depth
    grad_accum = 4
    global_batch_tok = world_size * micro_batch * grad_accum * seq_len
    total_steps = target_tokens // global_batch_tok
    # Round 2.1 continues from round-2's cosine-decayed end (LR was 3e-5 at
    # step 12207). Don't re-spike to peak; warmup briefly to 1e-4 (3.3x the
    # round-2 floor) then cosine decay back to 1e-5. This treats round 2.1
    # as a second annealing phase in the new ACT-bypassed regime.
    warmup_steps = 200
    lr = 1e-4
    wd = 0.1
    log_every = 5
    ckpt_every = 100
    ckpt_dir = "checkpoints_3b_varT_act_v2"
    ...
    # Round 2.1: bypass ACT during training. The recurrent loop runs all T
    # iterations and returns h_T; gradient flows through every iteration.
    # Applied here, before FSDP wrap, so the patched method is what FSDP sees.
    ...
    if existing_ckpts:
        # Continuing a round-2.1 run that already started: load latest sharded
        # ckpt as usual.
        latest = existing_ckpts[-1]
        if master:
            logger.info(f"Resuming round 2.1 checkpoint: {latest}")
        start_step = load_checkpoint(model, optimizer, latest, ddp)
        if master:
            logger.success(f"Resumed at step {start_step}")
    else:
        # First launch of round 2.1: bootstrap model weights from round-2's
        # final consolidated full-state-dict. Optimizer is fresh — round 2.1
        # uses a different LR schedule and has only the 200-step warmup, so
        # we want clean Adam moments. Step counter starts at 0 (round 2.1's
        # cosine schedule is independent of round 2's).
        round2_full = "checkpoints_3b_varT_fast/step_0012207_full.pt"
        if os.path.exists(round2_full):
            if master:
                logger.info(f"Bootstrapping from round-2 final: {round2_full}")
            # Model-only load: consolidated ckpt has no optimizer state, and
            # round 2.1 wants fresh Adam moments anyway (new LR schedule).
            bootstrap_model_weights(model, round2_full, ddp)
            start_step = 0
            if master:
                logger.success(
                    "Loaded round-2 weights; starting round-2.1 step counter at 0"
                )
        else:
            if master:
                logger.warning(
                    f"No prior ckpt and no round-2 bootstrap at {round2_full}; "
