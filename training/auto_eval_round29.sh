#!/usr/bin/env bash
# auto_eval_round29.sh
#
# Watcher that runs on kebab-spark.lan. Waits for round 2.9 training
# (T_FIXED=1 (no-recurrence) from r2 collapsed) to print "Training complete." in
...
#   7. build_paper_section7.py -> docs/paper/round29_results.md
...
log "round 2.9 final ckpt step=${STEP}"
...
for HOST in kebab-spark-200g kebab-gx10-200g kebab-gx10-2-200g kebab-gx10-3-200g; do
  ...
  ssh -q -o StrictHostKeyChecking=accept-new "alexm@$HOST" \
    "rm -f /tmp/consolidate_round29_r${RANK}.log; cd ~/OpenMythos && nohup env $NCCL_BASE NCCL_IB_GID_INDEX=${GID} \
     torchrun --nnodes=4 --nproc_per_node=1 --node_rank=${RANK} \
       ...
       >/tmp/consolidate_round29_r${RANK}.log 2>&1 </dev/null & disown" &
  RANK=$((RANK+1))
done
wait
...
while [ ! -s "$DST_PATH" ] || [ -f "${DST_PATH}.tmp" ]; do
  if [ "$(date +%s)" -gt "$DEADLINE" ]; then
    ...
    tail -n 40 /tmp/consolidate_round29_r0.log 2>&1 | tee -a "$LOG"
    exit 1
  fi
  sleep 5
done
...
mv -v "$DOCS_DIR/training_curve.png" "$PAPER_FIG_DIR/round29_training_curve.png" 2>&1 | tee -a "$LOG"
mv -v "$DOCS_DIR/loss_by_T.png" "$PAPER_FIG_DIR/round29_loss_by_T.png" 2>&1 | tee -a "$LOG"

PAPER_FRAG=$DOCS_DIR/paper/round29_results.md
...
[$(date '+%F %T')] auto_eval_round29 pipeline complete.
  ...
  Training curves:         ${PAPER_FIG_DIR}/round29_*.png
  Paper section fragment:  ${PAPER_FRAG}

Next: round 2.9 (T_FIXED=1 (no-recurrence) from r2 collapsed) is the symmetric
