...
| 7a | row #7 recurs on gx10 and gx10-2 after every reboot — `nmcli` persistence does NOT survive across all NM regeneration paths on these two nodes (sibling gx10-3 has direct `nameserver 192.168.210.1` lines in `/etc/resolv.conf` and is immune). 2026-05-22 manifestation: 10.5-min crash loop, 9 supervisor auto-relaunches overnight, looked like an NCCL `_ALLGATHER_BASE` hang but was ranks 1 & 2 crashing on `huggingface.co` DNS-resolution failure for fineweb-edu metadata, then ranks 0 & 3 stranded at the next allgather waiting for them. NCCL hang is symptomatic; logs from the gx10 ranks show the actual `requests.exceptions.ConnectionError: NameResolutionError`. | re-apply the row-#7 commands on the affected nodes; also bake `HF_DATASETS_OFFLINE=1 HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1` into `NCCL_BASE` in `training/launch_3b.sh` so a future DNS regression on any node doesn't crash training. The `HF_*_OFFLINE` knobs only need the dataset to already be in the local HF cache (which it always is for resume-from-checkpoint runs). | gx10, gx10-2; training/launch_3b.sh |
| 7b | row 7a's `nmcli` fix recurred a second time after 2-3 days (2026-05-27, again silently broke training during a multi-hour overnight run; same 3-node pattern: spark, gx10, gx10-2 all regressed, gx10-3 untouched). Some NM/DHCP-renew event keeps stomping the `ipv4.dns` setting back to whatever the DHCP server hands out. **Durable fix** mirrors gx10-3's working setup exactly: replace the systemd-resolved stub symlink at `/etc/resolv.conf` with a static regular file containing direct `nameserver 192.168.210.1\nnameserver 8.8.8.8`. systemd-resolved keeps running for resolvectl callers but no longer manages `/etc/resolv.conf`; NetworkManager does NOT overwrite a static file (no `rc-manager=` config = leave-it-alone default). gx10-3 has held this config since 2026-05-07 without regression. Applied to spark + gx10 + gx10-2 on 2026-05-28; previous symlinks were `/etc/resolv.conf -> ../run/systemd/resolve/stub-resolv.conf` (dating from 2025-08-06 install). Command: `sudo rm /etc/resolv.conf && sudo bash -c "cat > /etc/resolv.conf <<EOF\nnameserver 192.168.210.1\nnameserver 8.8.8.8\nEOF"`. | spark, gx10, gx10-2 |
